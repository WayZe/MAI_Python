...
# Как должна работать программа: на вход даете файл с текстом песни, 
# на выходе получаете самые часто встречающиеся слова. 
...
with open("/Users/andreymakarov/Downloads/mai_python_2019/04 Lambdas/pinksweater.txt", encoding="utf-8-sig") as f:
	text = f.read()
...
# TODO 1: разбейте текст на слова способом, который предпочитаете, 
# и сохраните результат в переменную words
words = list(text_clean.split(" "))

# TODO 2: напишите lambda-функцию для удаления пустых слов 
# (подсказка: также используйте функцию filter)
words = list(filter(lambda x: x != "", words))

# TODO 3: оцените состав слов: отсортируйте все слова песни по длине 
# по убыванию при помощи lambda-функции, как мы делали на лекции
...
for pair in words_count:
    if len(pair[1]) > 3:
        ...
        # и если нет - создать его удобным для вас способом, как мы делали для квартир
        if pair[0] not in words_dict.keys():
            words_dict[pair[0]] = list()
        words_dict[pair[0]].append(pair[1])

# TODO 6: выведите наиболее часто встречающиеся слова 
# (например, первую половину списка со статистикой)
# (придумайте сами, как вы будете рассчитывать, какой индекс у половины списка)
# к этому моменту ваши данные должны выглядеть примерно так 
# (для песни из файла pinksweater.txt):
# 10: ['бывает', 'есть']
