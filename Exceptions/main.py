# МАТЧАСТЬ множества - тип данных, который позволяет быстро сравнивать наборы элементов очень большого размера.
# Множества оптимизирует сам питон, поэтому доступа по индексу и порядка элементов, как в списке, там нет. Также
# множество может содержать только уникальные эначения (вспомните ключи словаря) множества
# https://pythonworld.ru/tipy-dannyx-v-python/mnozhestva-set-i-frozenset.html диаграмма Венна
# https://ru.wikipedia.org/wiki/Диаграмма_Венна очень понятная визуализация теории множеств :)
# https://static.tildacdn.com/tild3761-6332-4239-a166-306134646164/20180123_084944.png

# Исключение - описание типов исключений и пример работы с ними
...
# пусть у нас есть несколько станций метро, рядом с которыми мы хотим найти квартиру. Проверим, есть ли какие-то из
# них в выгрузке с ЦИАНа, при помощи пересечения множеств
test_set = set(["Перово", "Новогиреево", "Выхино"])
print()
print(f"Пересечение множеств: \n{subway_set & test_set}")
print(f"Пересечение множеств: \n{subway_set.intersection(test_set)}") # то же самое

# найдем метро из первого множества, которых нет во втором. Обратите внимание, что здесь уже играет значение,
# для какого множества мы вызываем функцию
print()
...
# при помощи функции assert мы можем проверить, выполняется ли какое-либо условие. В отличие от простой проверки вида
# if "Черкизовская" in test_set, которая вернет False, функция assert сгенерирует исключение, если условие не
# выполнится (результат False)
...
try:
    ...
    # генерируем исключение вручную, чтобы увидеть, как работает код. Если мы хотим увидеть пояснительный текст,
    # необходимо ввести его как параметр
...
# этот блок срабатывает всегда, не зависимо от того, было ли исключение или нет. Обратите внимание: даже если в блоке
# перехвата исключения была команда выхода из цикла, блок finally выполнится все равно
finally:
...
while True:
    ...
    # пользователь должен ввести два значения через запятую. Ввод через input() - всегда строка, поэтому придется
    # разбить ее функцией split(",")
    ...
    # здесь есть также два варианта действий: 1) проверить количество параметров len(params) == 2 либо использовать
...
# для того, чтобы создать собственное исключение, необходимо создать класс и унаследовать его от какого-то системного
# исключения
print()
